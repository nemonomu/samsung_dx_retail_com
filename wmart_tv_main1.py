...
class WalmartTVCrawler:
    def __init__(self):
        ...
        self.max_skus = self.config.get_constant_int('max_skus', 'wmart_tv_main1', default=9999)
    ...
    def scrape_page(self, url, page_number, retry_count=0):
        ...
        max_retries = self.config.get_retry('max_retries', 'wmart_tv_main1', default=2)
    ...
    def run(self):
        ...
        try:
            ...
            page_start, page_end = self.config.get_page_range('wmart_tv_main1')

            print("="*80)
            print(f"Walmart TV Crawler [Part 1: Pages {page_start}-{page_end}] - Starting (undetected-chromedriver)")
            ...
            max_page_retries = self.config.get_retry('max_page_retries', 'wmart_tv_main1', default=2)
            retry_wait = self.config.get_timing_range('retry_wait') or (10, 15)
            between_pages = self.config.get_timing_range('between_pages', 'wmart_tv_main1') or (8, 12)
            ...
            try:
                ...
                with open(os.path.join(result_dir, "wmart_tv_main1.json"), "w") as f:
...
if __name__ == "__main__":
    ...
    log_filename = "C:\\samsung_dx_retail_com\\log\\" + datetime.now().strftime("%Y%m%d_%H%M%S") + "_main1.txt"
