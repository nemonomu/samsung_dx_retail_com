...
class BestBuyBSRCrawler(BaseCrawler):
    ...
    def run(self):
        ...
        try:
            ...
            # 통계 출력
            print(f"\n{'='*50}")
            print(f"[통계] 수집: {self.stats['collected']}, 중복제거: {self.stats['duplicates']}, 키워드필터: {self.stats['keyword_filtered']}, OpenBox: {self.stats['openbox_filtered']}, 비제품: {self.stats['non_product']}, UPDATE: {self.stats['updated']}, INSERT: {self.stats['inserted']}")
...
def main():
    ...
    crawler = BestBuyBSRCrawler(test_mode=True, time_offset_hours=time_offset)
    crawler.run()
    input("\n[완료] 엔터를 누르면 종료됩니다...")
