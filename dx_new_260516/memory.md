...
- BestBuy PDP may split the date phrase and the free-shipping text into separate HTML nodes, for example `Delivery As soon as Wed, May 20` plus `FREE delivery to ZIP`. Recombine these into one stored value when the page-visible fulfillment area shows free delivery/shipping/pickup.
