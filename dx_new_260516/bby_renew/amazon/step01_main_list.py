...
def fetch_once(page, attempt):
    template = target_url("main")
...
def save_attempt(result):
    ...
    request_path.write_text(
        json.dumps({"page": page, "attempt": result["attempt"], "url": result["url"], "params": REQUEST_PARAMS}, indent=2),
        encoding="utf-8",
    )
...
def fetch_page(page):
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        result = fetch_once(page, attempt)
...
def fetch_cached_page(page):
    ...
    return {
        ...
        "url": meta.get("url", url_for_page(target_url("main"), page)),
...
def write_page_summary(path, results):
    fieldnames = ["page", "url", "status_code", "attempts", "elapsed_seconds", "bytes", "item_count", "x_request_cost", "error"]
    with path.open("w", encoding="utf-8-sig", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames, extrasaction="ignore")
        writer.writeheader()
