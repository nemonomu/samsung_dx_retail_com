...
cd /d "%~dp0"
set "BBY_ROOT=%CD%"
set "CHAIN_ORDER=BBY:TV / LOWES:REF LDY"
...
set "CHAIN_LOG=%CHAIN_LOG_DIR%\bby_tv_lowes_ref_ldy_%CHAIN_TS%.log"

echo ==================================================
echo BBY TV + Lowes REF+LDY daily task started
...
>>"%CHAIN_LOG%" echo ==================================================
if "%FAILURE_COUNT%"=="0" (
  echo BBY TV + Lowes REF+LDY daily task completed successfully
  >>"%CHAIN_LOG%" echo BBY TV + Lowes REF+LDY daily task completed successfully
) else (
  echo BBY TV + Lowes REF+LDY daily task completed with failures
  echo failed_count=%FAILURE_COUNT%
  echo failures=%FAILURES%
  >>"%CHAIN_LOG%" echo BBY TV + Lowes REF+LDY daily task completed with failures
...
echo task_bat=%BBY_ROOT%\_bby_daily_task.bat
echo ==================================================
>>"%CHAIN_LOG%" echo.
>>"%CHAIN_LOG%" echo ==================================================
>>"%CHAIN_LOG%" echo BestBuy %CATEGORY% daily task started
>>"%CHAIN_LOG%" echo task_bat=%BBY_ROOT%\_bby_daily_task.bat
>>"%CHAIN_LOG%" echo ==================================================
call :clear_bestbuy_env
if "%BBY_TV_LOWES_REF_LDY_DRY_RUN%"=="1" (
