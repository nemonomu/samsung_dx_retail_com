...
def migrate_wmart_tv_main_crawl():
    try:
        ...
        print("="*80)
        print("wmart_tv_main_crawl 테이블 마이그레이션")
        print("="*80)
        ...
        cursor.execute("""
            ...
            WHERE table_name = 'wmart_tv_main_crawl'
            AND column_name = 'account_name'
        """)

        if not cursor.fetchone():
            cursor.execute("""
                ALTER TABLE wmart_tv_main_crawl
        ...
        # 2. order → main_rank 이름 변경
        print("\n[2] order → main_rank 컬럼 이름 변경 중...")
        cursor.execute("""
            SELECT column_name
            FROM information_schema.columns
            WHERE table_name = 'wmart_tv_main_crawl'
        ...
        conn.commit()
        print("\n" + "="*80)
        print("[SUCCESS] wmart_tv_main_crawl 테이블 마이그레이션 완료!")
        print("="*80)

        # 변경 결과 확인
        print("\n[확인] wmart_tv_main_crawl 테이블 컬럼:")
        cursor.execute("""
