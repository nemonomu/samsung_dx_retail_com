...
class BestBuyTVDetailCrawler(BaseCrawler):
    ...
    def load_tv_xpaths(self, account_name, page_type):
        """TV 전용 XPath 로드 (tv_xpath_selectors 테이블)"""
        try:
            cursor = self.db_conn.cursor()
            cursor.execute("""
                SELECT data_field, xpath FROM tv_xpath_selectors
            ...
            if not rows:
                print(f"[ERROR] No XPaths found in tv_xpath_selectors (account={account_name}, page_type={page_type})")
                return False
            ...
            print(f"[INFO] TV XPaths loaded: {len(self.xpaths)} fields")
            return True

        except Exception as e:
            print(f"[ERROR] load_tv_xpaths failed: {e}")
            traceback.print_exc()
            return False
    ...
    def initialize(self):
        ...
        # XPath 로드 (DB 조회)
        if not self.load_tv_xpaths(self.account_name, self.page_type):
            return False
    ...
    def crawl_detail(self, product):
        ...
        try:
            ...
            # ========== 6단계: 리뷰 섹션 스크롤 → 별점 / 리뷰수 / top_mentions / recommendation_intent 추출 ==========
            star_rating = None
            ...
            if top_count_of_reviews:
                if 'not yet reviewed' in top_count_of_reviews.lower():
                    is_no_reviews = True
                elif 'reviews from' in top_count_of_reviews.lower():
                    is_no_reviews = True
            ...
            if is_no_reviews:
                ...
                # 1차: DOM에서 먼저 찾기 (3단계에서 이미 리뷰 섹션으로 스크롤됨)
                ...
                # 3차: 리뷰 버튼 못 찾으면 URL로 직접 접속 (sku_number 사용)
                if not review_button_found and sku_number:
                    try:
                        ...
                        if review_url:
                            ...
                            print(f"[WARNING] 리뷰 URL 생성 실패 - URL 형식 미지원: {current_url}")
                    except Exception as e:
                ...
                if review_button_found:
                    try:
                        ...
                        if detailed_review_xpath:
                            ...
                            # top_mentions fallback: 리뷰 페이지의 pros mentioned/cons mentioned에서 추출
                            if top_mentions is None:
                                ...
                                if top_mentions_fallback_raw:
                                    top_mentions = ', '.join([re.sub(r'\s*\(\d+\)\s*', '', item).strip() for item in top_mentions_fallback_raw.split(', ')])
        ...
        try:
            ...
            insert_query = f"""
                INSERT INTO {table_name} (
                    ...
                    detailed_review_content, top_mentions,
            ...
            values = (
                product.get('item'), self.account_name, product.get('page_type'),
                ...
                product.get('detailed_review_content'), product.get('top_mentions'),
