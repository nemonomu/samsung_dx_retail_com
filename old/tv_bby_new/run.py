...
PRODUCT_LIST_TABLE = "bby_tv_product_list_v2_test"
FINAL_PATTERN = "tv_retail_com_*.csv"
PRODUCT_LIST_PATTERN = "bby_tv_product_list_*.csv"
...
def log(message):
    print(f"[TV {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {message}", flush=True)
...
def main():
    parser = argparse.ArgumentParser(description="Repair BestBuy TV sample CSV and optionally load Unsan default tables.")
    ...
    final_out = OUTPUT_DIR / f"tv_retail_com_bestbuy_complete_{stamp}.csv"
    product_out = OUTPUT_DIR / f"bby_tv_product_list_{stamp}.csv"
