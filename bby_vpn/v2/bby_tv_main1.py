...
class BestBuyTVCrawler:
    def __init__(self):
        ...
        self.audit_log_path = os.path.join(self.csv_output_dir, 'bby_tv_main1_audit.jsonl')
        self.saved_urls = set()
    ...
    def scrape_page(self, url, page_number):
        ...
        try:
            ...
            self.rate_limiter.wait(url, reason='listing_page')
            self.page.get(url)
            self.browser_diagnostics.snapshot(self.page, url, f'listing_page_{page_number}_after_get')
            self.network_diagnostics.snapshot(self.page, url, f'listing_page_{page_number}_after_get')
    ...
    def run(self):
        ...
        try:
            ...
            print(f"\n[INFO] Crawl audit JSONL saved at: {self.audit_log_path}")
