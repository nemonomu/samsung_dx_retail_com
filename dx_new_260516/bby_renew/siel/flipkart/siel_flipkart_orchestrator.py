...
STEPS = [
    Step(1, "main_list", "Flipkart relevance listing"),
    Step(3, "bsr_list", "Flipkart popularity listing"),
    Step(8, "detail_enrichment", "Flipkart PDP detail/review enrichment"),
    Step(14, "csv_export", "Export JSONL run output to final/product-list CSV"),
]


def latest_jsonl(product: str) -> Path | None:
    pattern = f"siel_flipkart_{product.lower()}_run_*.jsonl"
    matches = sorted(LOGS_DIR.glob(pattern), key=lambda p: p.stat().st_mtime, reverse=True)
    return matches[0] if matches else None


def print_steps():
    print("SIEL Flipkart pipeline steps:")
    for step in STEPS:
        print(f"  {step.key} {step.name:<18} {step.description}")


def main() -> int:
    parser = argparse.ArgumentParser(description="SIEL Flipkart crawler orchestrator")
    ...
    parser.add_argument("--max-rank-main", type=int, default=None)
    parser.add_argument("--max-rank-bsr", type=int, default=None)
    ...
    if not args.export_latest and not args.jsonl_path:
        ...
        if args.max_rank is not None:
            command.extend(["--max-rank", str(args.max_rank)])
        if args.max_rank_main is not None:
            command.extend(["--max-rank-main", str(args.max_rank_main)])
        if args.max_rank_bsr is not None:
            command.extend(["--max-rank-bsr", str(args.max_rank_bsr)])
