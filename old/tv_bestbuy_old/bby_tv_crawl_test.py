...
class BestBuyTVIntegratedCrawlerTest:
    ...
    def run(self):
        ...
        if not self.batch_id:
            self.batch_id = self.base_crawler.generate_batch_id(self.account_name, test_mode=True, time_offset_hours=self.time_offset_hours)
        ...
        try:
            ...
            if self._should_run('main'):
                ...
                try:
                    success = BestBuyTVMainCrawler(test_mode=True, batch_id=self.batch_id, time_offset_hours=self.time_offset_hours).run()
            ...
            if self._should_run('bsr'):
                ...
                stage_start = time.time()
                try:
                    success = BestBuyTVBSRCrawler(test_mode=True, batch_id=self.batch_id, time_offset_hours=self.time_offset_hours).run()
            ...
            if self._should_run('pmt'):
                ...
                stage_start = time.time()
                try:
                    success = BestBuyTVPromotionCrawler(test_mode=True, batch_id=self.batch_id, time_offset_hours=self.time_offset_hours).run()
            ...
            if self._should_run('trend'):
                ...
                stage_start = time.time()
                try:
                    success = BestBuyTVTrendCrawler(test_mode=True, batch_id=self.batch_id, time_offset_hours=self.time_offset_hours).run()
            ...
            stage_start = time.time()
            try:
                success = BestBuyTVDetailCrawler(batch_id=self.batch_id, test_mode=True, time_offset_hours=self.time_offset_hours).run()
...
def main():
    ...
    crawler = BestBuyTVIntegratedCrawlerTest(resume_from=args.resume_from, batch_id=args.batch_id, time_offset_hours=time_offset)
    success = crawler.run()
