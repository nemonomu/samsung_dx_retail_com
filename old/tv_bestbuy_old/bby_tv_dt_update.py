...
class BestBuyTVDetailUpdateCrawler(BaseCrawler):
    """
    ...
    def load_product_list(self):
        ...
        try:
            ...
            print(f"[INFO] Loaded {len(product_list)} products ({mode_desc}) from tv_retail_com")
            return product_list
    ...
    def crawl_detail(self, product):
        ...
        try:
            ...
            combined_data.update({
                ...
                'crawl_datetime': (datetime.now() + timedelta(hours=TIME_OFFSET_HOURS)).strftime('%Y-%m-%d %H:%M:%S')
            })
            ...
            if "error page detected" in str(e).lower():
                raise
            product['crawl_datetime'] = (datetime.now() + timedelta(hours=TIME_OFFSET_HOURS)).strftime('%Y-%m-%d %H:%M:%S')
            return product
    ...
    def update_retail_com(self, product):
        ...
        try:
            ...
            params.append(product.get('crawl_datetime'))
            ...
            update_query = f"UPDATE tv_retail_com SET {', '.join(updates)} WHERE id = %s"
    ...
    def run(self):
        ...
        try:
            ...
            print(f"[RATE-LIMIT] 수집 시작: {(crawl_start_time + timedelta(hours=TIME_OFFSET_HOURS)).strftime('%H:%M:%S')}")
                ...
                try:
                    ...
                    if "error page detected" in error_msg:
                        if not first_error_logged:
                            ...
                            print(f"[RATE-LIMIT] 첫 차단 발생: {(datetime.now() + timedelta(hours=TIME_OFFSET_HOURS)).strftime('%H:%M:%S')} (수집 시작 후 {int(elapsed)}초, {i-1}건 수집)")
            ...
            print(f"[DONE] Processed: {len(product_list)}, Updated: {total_updated}, Table: tv_retail_com, batch_id: {self.batch_id}")
            return True
...
def fetch_today_batch_ids():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT DISTINCT batch_id FROM tv_retail_com "
...
def main():
    ...
    if time_offset_arg is not None:
        ...
        print("   7 : 기본값 (서버 시간 + 7시간 = 미국 동부 자정에 맞춤)")
        print("   0 : 현재 서버 시간 그대로 사용")
        print("  -7 : 서버 시간 - 7시간 (뺄셈 기호 가능)")
        print("  (입력 예시: '1'은 1시간 더함, '-6'은 6시간 뺌)")
        offset_input = input("오프셋 시간 입력 (기본: 7): ").strip()
        try:
            TIME_OFFSET_HOURS = int(offset_input) if offset_input else 7
        except ValueError:
            TIME_OFFSET_HOURS = 7
    ...
    if not batch_id:
        ...
        if today_batch_ids:
            print(f"\n오늘({(datetime.now() + timedelta(hours=TIME_OFFSET_HOURS)).strftime('%Y-%m-%d')}) BestBuy TV batch_id 목록:")
            ...
            print(f"오늘({(datetime.now() + timedelta(hours=TIME_OFFSET_HOURS)).strftime('%Y-%m-%d')}) BestBuy TV batch_id가 없습니다.")
    ...
    crawler = BestBuyTVDetailUpdateCrawler(batch_id=batch_id, start_id=start_id, mode=mode)
