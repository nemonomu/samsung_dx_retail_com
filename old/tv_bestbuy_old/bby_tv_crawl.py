...
class BestBuyTVIntegratedCrawler:
    ...
    def __init__(self, resume_from=None, batch_id=None, time_offset_hours=0):
        """
        Args:
            resume_from: 재시작 단계 ('main'/'bsr'/'pmt'/'trend'/'detail'/None)
            batch_id: 재시작 시 사용할 배치 ID
            time_offset_hours: 시간 오프셋 (기본값: 0)
        """
    ...
    def run(self):
        ...
        if not self.batch_id:
            self.batch_id = self.base_crawler.generate_batch_id(self.account_name, time_offset_hours=self.time_offset_hours)
        ...
        try:
            ...
            if self._should_run('main'):
                ...
                try:
                    success = BestBuyTVMainCrawler(test_mode=False, batch_id=self.batch_id, time_offset_hours=self.time_offset_hours).run()
            ...
            # STEP 2: BSR
            if self._should_run('bsr'):
                print(f"\n[STEP 2/5] BSR Crawler...")
                stage_start = time.time()
                try:
                    success = BestBuyTVBSRCrawler(test_mode=False, batch_id=self.batch_id, time_offset_hours=self.time_offset_hours).run()
            ...
            # STEP 3: Promotion
            if self._should_run('pmt'):
                print(f"\n[STEP 3/5] Promotion Crawler...")
                stage_start = time.time()
                try:
                    success = BestBuyTVPromotionCrawler(test_mode=False, batch_id=self.batch_id, time_offset_hours=self.time_offset_hours).run()
            ...
            # STEP 4: Trend
            if self._should_run('trend'):
                print(f"\n[STEP 4/5] Trend Crawler...")
                stage_start = time.time()
                try:
                    success = BestBuyTVTrendCrawler(test_mode=False, batch_id=self.batch_id, time_offset_hours=self.time_offset_hours).run()
            ...
            if self._should_run('detail'):
                ...
                stage_start = time.time()
                try:
                    success = BestBuyTVDetailCrawler(batch_id=self.batch_id, test_mode=False, time_offset_hours=self.time_offset_hours).run()
...
def main():
    ...
    crawler = BestBuyTVIntegratedCrawler(resume_from=args.resume_from, batch_id=args.batch_id, time_offset_hours=time_offset)
    success = crawler.run()
