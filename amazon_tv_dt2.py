        ...
        for attempt in range(2):
            try:
                ...
                if 'signin' in current_url or 'ap/signin' in current_url:
                    ...
                    try:
                        current_email = AMAZON_ACCOUNTS[self.current_account]['email']
                        current_password = AMAZON_ACCOUNTS[self.current_account]['password']
                        login_to_amazon_dp(self.page, current_email, current_password)
                        save_cookies_dp(self.page, self.current_cookie_file)
                        review_url = f"https://www.amazon.com/product-reviews/{asin}/ref=cm_cr_dp_d_show_all_btm?ie=UTF8&reviewerType=all_reviews"
                        self.page.get(review_url)
                        time.sleep(random.uniform(8, 12))
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                with open(os.path.join(result_dir, "amazon_tv_dt2.json"), "w") as f:
                    json.dump({
                        "target_count": target,
