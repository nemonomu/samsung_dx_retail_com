...
class BestBuyTVMainCrawler(BaseCrawler):
    ...
    def initialize(self):
        ...
        if not self.load_xpaths(self.account_name, self.page_type, 'SEA', 'TV'):
            print(f"[ERROR] Initialize failed: TV XPath load failed (account={self.account_name}, page_type={self.page_type})")
            return False
        ...
        try:
            ...
            while total_products < target_products and page_num <= self.max_pages:
                ...
                if not products:
                    ...
                    # product_url NULL 비율 체크 → 대부분 NULL이면 에러 페이지로 판단
                    null_url_count = sum(1 for p in products if not p.get('product_url'))
                    if null_url_count >= 3:
...
def main():
    ...
    crawler = BestBuyTVMainCrawler(test_mode=True, time_offset_hours=time_offset)
    crawler.run()
    input("\n[완료] 엔터를 누르면 종료됩니다...")
