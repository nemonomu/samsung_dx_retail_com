...
def fetch_page(page):
    ...
    if cached:
        return cached
    url = url_for_page(target_url("bsr"), page)
    last_result = None
...
def fetch_cached_page(page):
    ...
    return {
        ...
        "url": meta.get("url", url_for_page(target_url("bsr"), page)),
...
def save_attempt(result):
    ...
    meta_path = unit_dir / f"page_{page:03d}_meta.json"
    request_path.write_text(json.dumps({"page": page, "attempt": result["attempt"], "url": result["url"], "params": REQUEST_PARAMS}, indent=2), encoding="utf-8")
