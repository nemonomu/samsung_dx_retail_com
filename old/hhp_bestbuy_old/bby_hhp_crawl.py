    ...
    def run(self):
        ...
        try:
            ...
            if not self.resume_from or self.resume_from == 'main':
                ...
                try:
                    success = BestBuyMainCrawler(test_mode=False, batch_id=self.batch_id, time_offset_hours=self.time_offset_hours).run()
            ...
            if not self.resume_from or self.resume_from in ['main', 'bsr']:
                ...
                stage_start = time.time()
                try:
                    success = BestBuyBSRCrawler(test_mode=False, batch_id=self.batch_id, time_offset_hours=self.time_offset_hours).run()
            ...
            if not self.resume_from or self.resume_from in ['main', 'bsr', 'trend']:
                ...
                stage_start = time.time()
                try:
                    success = BestBuyTrendCrawler(test_mode=False, batch_id=self.batch_id, time_offset_hours=self.time_offset_hours).run()
            ...
            stage_start = time.time()
            try:
                success = BestBuyDetailCrawler(batch_id=self.batch_id, test_mode=False, time_offset_hours=self.time_offset_hours).run()
...
def main():
    ...
    crawler = BestBuyIntegratedCrawler(resume_from=args.resume_from, batch_id=args.batch_id, time_offset_hours=time_offset)
    success = crawler.run()
